...
export async function getAllNotices(
  ...
  const query = await client
    ...
    .hasAncestor(client.key([KINDS.city, citySlug]))
    .filter(new PropertyFilter("date", ">=", afterDay));
  const [res] = await query.run();
  return res;
}

export async function getNoticeFor(
  citySlug: string,
  day: string,
): Promise<Notice | null> {
  const [res] = await client.get(
    client.key([KINDS.city, citySlug, KINDS.notice, day]),
  );
  return res ?? null;
}
...
export const noticeLoader = new DataLoader(
  async (days: ReadonlyArray<[string, string]>) =>
    Promise.all(days.map(([citySlug, d]) => getNoticeFor(citySlug, d))),
);
// #endregion
