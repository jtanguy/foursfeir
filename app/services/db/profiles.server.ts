...
export const profileLoader = new DataLoader(
  async (userIds: ReadonlyArray<string>) => {
    ...
    const [results] = await client.get(
      userIds.map((u) => client.key([KINDS.profile, u])),
    );
    return userIds.map(
      (u) =>
        (results as Profile[]).find((p) => p[client.KEY].name === u) ?? null,
    );
  },
);
...
export async function getProfiles(): Promise<Profile[][]> {
  ...
  return profiles.map((profile) => ({
    ...profile,
    id: profile[client.KEY].name,
  }));
}

export function saveProfile(profile: Profile) {
  return client.save({
    key: client.key([KINDS.profile, emailToFoursfeirId(profile.email)]),
    data: profile,
  });
}
