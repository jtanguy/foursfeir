...
type ProfileEntity = Entity<ProfileEntityCreation>;
...
export class DatastoreProfileService implements ProfileService {
  ...
  loader = new DataLoader(async (userIds: ReadonlyArray<string>) => {
    const [results] = await this.client.get(
      userIds.map((u) => this.client.key([KINDS.profile, u]))
    );
    const profiles = results.map((raw: ProfileEntity) => ({
      ...raw,
      user_id: raw[this.client.KEY].name,
    }));
    return userIds.map(
      (u) =>
        profiles.find((p) => p.user_id === u) ??
        new Error(`Profile not found: ${u}`)
    );
  });
  ...
  async createProfile(data: Omit<Profile, "created_at">): Promise<Profile> {
    ...
    const entity: ProfileEntityCreation = {
      ...data,
      id: data.user_id,
    };
  ...
  async searchProfiles(
    ...
    return this.fuseInstance
      .search(pattern)
      .map((result) => ({ profile: result.item, score: result.score! }));
  }
